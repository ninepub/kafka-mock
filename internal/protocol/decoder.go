...
type PacketDecoder interface {
	...
	// Added ex
	RawBytes(length int) ([]byte, error)
	Subset(length int) (*ByteDecoder, error)
...
// Added Ex
func (d *ByteDecoder) RawBytes(length int) ([]byte, error) {
	if length < 0 {
		return nil, ErrInvalidByteSliceLength
...
func (d *ByteDecoder) Subset(length int) (*ByteDecoder, error) {
	buf, err := d.RawBytes(length)
	if err != nil {
		return nil, err
	}
...
func (d *ByteDecoder) PeekInt8(offset int) (int8, error) {
	const byteLen = 1
	if d.remaining() < offset+byteLen {
		return -1, ErrInsufficientData
	}
	return int8(d.b[d.off+offset]), nil
}

func (d *ByteDecoder) Varint() (int64, error) {
	tmp, n := binary.Varint(d.b[d.off:])
	if n == 0 {
		d.off = len(d.b)
		return -1, ErrInsufficientData
	}
